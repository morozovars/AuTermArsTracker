...
QStringList ars_tracker_backend::build_remote_file_candidates(const QString &filename) const
{
    QStringList candidates;

    auto append_candidate = [&candidates, this](const QString &root, const QString &name) {
        QString candidate = build_remote_file_path(root, name);

        if (candidate.isEmpty() == false && candidates.contains(candidate) == false)
        {
            candidates.append(candidate);
        }
    };

    append_candidate(active_session_remote_root, filename);
    append_candidate(active_session_name, filename);

    if (active_session_remote_root.startsWith('/') == false && active_session_remote_root.isEmpty() == false)
    {
        append_candidate(QString("/") + active_session_remote_root, filename);
    }

    if (active_session_name.startsWith('/') == false && active_session_name.isEmpty() == false)
    {
        append_candidate(QString("/") + active_session_name, filename);
    }

    append_candidate(QString(), filename);
    return candidates;
}
...
void ars_tracker_backend::enqueue_fixed_files()
{
    for (const QString& file_name : ars_tracker_fixed_files)
    {
        ...
        item.remote_candidates = build_remote_file_candidates(file_name);
        item.remote_candidate_index = 0;
        item.remote_file = item.remote_candidates.isEmpty() ? file_name : item.remote_candidates.first();
...
void ars_tracker_backend::enqueue_next_sensor_candidate()
{
    ...
    ars_tracker_download_item_t item;
    item.remote_candidates = build_remote_file_candidates(sensor_filename);
    item.remote_candidate_index = 0;
    item.remote_file = item.remote_candidates.isEmpty() ? sensor_filename : item.remote_candidates.first();
                                               ...
                                               QString *error_message)
{
    ...
    active_session_remote_root = session.remote_fs_root;
    active_destination_path = destination_path;

    qDebug() << "[ArsTracker][Export] Session selected"
             << "session_id=" << active_session_id
             << "session_name=" << active_session_name
             << "remote_root=" << active_session_remote_root
             << "destination=" << active_destination_path;
...
void ars_tracker_backend::request_next_download_or_finish()
{
    ...
    for (int i = 0; i < download_queue.length(); ++i)
    {
        if (download_queue.at(i).status == ARS_TRACKER_STATUS_PENDING)
        {
            ...
            qDebug() << "[ArsTracker][Export] Download attempt"
                     << "session_id=" << active_session_id
                     << "remote_root=" << active_session_remote_root
                     << "remote_file=" << item.remote_file
                     << "candidate_index=" << item.remote_candidate_index
                     << "candidate_count=" << item.remote_candidates.length()
                     << "local_temp=" << item.local_temp_file;
...
void ars_tracker_backend::handle_file_download_result(group_status status, const QString &error_message)
{
    ...
    qDebug() << "[ArsTracker][Export] Download result"
             << "session_id=" << active_session_id
             << "remote_root=" << active_session_remote_root
             << "remote_file=" << item.remote_file
             << "status=" << status
    ...
    {
        if (item.remote_candidate_index + 1 < item.remote_candidates.length())
        {
            ++item.remote_candidate_index;
            item.remote_file = item.remote_candidates.at(item.remote_candidate_index);
            item.status = ARS_TRACKER_STATUS_PENDING;
            item.error_text = "Trying alternate remote path";
            publish_export_file_rows();
            request_next_download_or_finish();
            return;
        }

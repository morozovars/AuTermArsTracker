...
#include "ars_tracker/ars_tracker_players_tab.h"
...
void plugin_mcumgr::setup(QMainWindow *main_window)
{
		...
		setup_ars_players_tab(tabWidget_orig);
...
void plugin_mcumgr::setup_ars_players_tab(QTabWidget *tabWidget_orig)
{
		if (tabWidget_orig == nullptr)
		{
				log_warning() << "Players tab setup skipped: tabWidget_orig is null";
				return;
		}
		log_debug() << "Players tab setup begin";
		for (int i = 0; i < tabWidget_orig->count(); ++i)
		{
				QWidget *w = tabWidget_orig->widget(i);
				const QString title = tabWidget_orig->tabText(i);
				const bool object_name_match =
						(w != nullptr && w->objectName() == "ars_tracker_players_tab");
				const bool title_match = (title.compare("Players", Qt::CaseInsensitive) == 0);
				if (object_name_match || title_match)
				{
						ArsTrackerPlayersTab *existing = qobject_cast<ArsTrackerPlayersTab *>(w);
						if (existing != nullptr)
						{
								ars_tracker_players_tab = existing;
						}
						return;
				}
		}
		if (ars_tracker_players_tab == nullptr)
		{
				ars_tracker_players_tab = new ArsTrackerPlayersTab(tabWidget_orig);
		}
		ars_tracker_players_tab->setObjectName("ars_tracker_players_tab");
		...
		{
				const QString title = tabWidget_orig->tabText(i);
				if (title.compare("Team", Qt::CaseInsensitive) == 0) team_index = i;
				if (title.compare("Sessions", Qt::CaseInsensitive) == 0) sessions_index = i;
		}
		int inserted = -1;
		if (team_index >= 0)
		{
				inserted = tabWidget_orig->insertTab(team_index + 1, ars_tracker_players_tab, QString("Players"));
		}
		else if (sessions_index >= 0)
		{
				inserted = tabWidget_orig->insertTab(sessions_index + 1, ars_tracker_players_tab, QString("Players"));
		}
		else
		{
				inserted = tabWidget_orig->addTab(ars_tracker_players_tab, QString("Players"));
		}
		log_debug() << "Players tab added index=" << inserted << "objectName=" << ars_tracker_players_tab->objectName();
		log_debug() << "Players tab initialized";
}
...
void plugin_mcumgr::on_selector_tab_currentChanged(int index)
{
	...
	if (selector_tab_root != nullptr && selector_tab_root->currentWidget() == ars_tracker_players_tab)
	{
			ars_tracker_players_tab->reloadPlayers("players-tab-active");
	}
		...
		for (; row_index < row_count; ++row_index)
		{
				...
				else
				{
						table_ars_trackers->setCellWidget(
								row_index, 1, make_placeholder_cell_widget("Left: not connected"));
						++cells_created;
				}
				if (pair.has_right)
				{
				...
				{
						table_ars_trackers->setCellWidget(
								row_index, 2, make_placeholder_cell_widget("Right: not connected"));
						++cells_created;
				}
